            ...
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-md-5 bg-white rounded-3 position-relative">
                        <form action="
                          @if ($edit==null)
                            {{ route('jabatan') }}/add/insert
                          @else
                            {{ route('jabatan') }}/{{ $edit->id }}/edit/update
                          @endif  
                        " method="POST">
                          @csrf
                            ...
                            <div class="form-group">
                              ...
                              <input type="number" class="form-control" id="prioritas" name="prioritas" value="{{ old('prioritas')??$edit->prioritas??"" }}" placeholder="Prioritas"> 
                              @error('prioritas')
                              <div class="validation-error">
                                {{ $message }}
                              </div>
